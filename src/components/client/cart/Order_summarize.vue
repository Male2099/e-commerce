...
<template>
    ...
    <form action="#" @submit="onApplyPromocode" class="w-full">
        <div v-if="enableApplyPromocode" class="w-full mb-2 flex flex-col gap-3 font-bold text-[1.1rem] text-gray-500">
            ...
            <input type="text" v-model="promoCode" name="promocode" required id="promocode" placeholder="Enter code here"
                class="w-full px-2 rounded h-[2.5rem] text-sm font-normal">
        </div>
    ...
    <div class="w-full mb-2 flex  justify-center font-bold text-[1.1rem] text-gray-500">
        <button @click="$emit('checkout_step1'); enableCheckout = false" v-if="enableCheckout || backToCart"
